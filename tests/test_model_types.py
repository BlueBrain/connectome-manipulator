...
def test_LinDelayModel():
    ...
    with setup_tempdir(__name__) as tempdir:
        ...
        d_coef = [0.1, 0.003]
        d_min = 0.2
        model_dict = {'model': 'LinDelayModel', 'delay_mean_coefs': d_coef, 'delay_std': 0.0, 'delay_min': d_min}
        ...
        d_std = 0.5
        model_dict = {'model': 'LinDelayModel', 'delay_mean_coefs': d_coef, 'delay_std': d_std, 'delay_min': d_min}
        model = test_module.AbstractModel.model_from_dict(model_dict)
        dist = 1000
        N = 10000
        ...
        model2 = test_module.AbstractModel.model_from_file(os.path.join(tempdir, model_name + '.json'))
        assert model.get_param_dict() == model2.get_param_dict()
        assert sorted(model.get_data_dict().keys()) == sorted(model2.get_data_dict().keys())
        assert np.all([np.array_equal(model.get_data_dict()[k], model2.get_data_dict()[k]) for k in model.get_data_dict().keys()])
...
def test_ConnPropsModel():
    ...
    with setup_tempdir(__name__) as tempdir:
        ...
        model_dict = {'model': 'ConnPropsModel',
                      'src_types': ['L4_MC'],
                      'tgt_types': ['L4_MC'],
                      'prop_stats': {'n_syn_per_conn': {'L4_MC': {'L4_MC': {'type': 'constant',
                                                                            'mean': N_syn,
                                                                            'dtype': 'int'}}},
                                     'conductance': {'L4_MC': {'L4_MC': {'type': 'normal',
                                                                         'mean': cond_mean,
                                                                         'std': cond_std,
                                                                         'std-within': 0.0,
                                                                         'dtype': 'float'}}}}}
...
def test_ConnProb2ndOrderExpModel():
    ...
    with setup_tempdir(__name__) as tempdir:
        ...
        assert p.shape[0] == src_pos.shape[0] and p.shape[1] == tgt_pos.shape[0]
        assert np.all(p[0, :] == scale * np.exp(-exponent * dist)) # Exponential dist.-dep. conn. prob.
...
def test_ConnProb3rdOrderExpModel():
    ...
    with setup_tempdir(__name__) as tempdir:
        ...
        model_dict = {'model': 'ConnProb3rdOrderExpModel', 'scale_P': scale[0], 'scale_N': scale[1],
                      'exponent_P': exponent[0], 'exponent_N': exponent[1], 'bip_coord': tgt_pos.shape[1] - 1}
        model = test_module.AbstractModel.model_from_dict(model_dict)
        p = model.apply(src_pos=src_pos, tgt_pos=tgt_pos)
        dist = np.sqrt(np.sum(tgt_pos**2, 1))
        P_sel = tgt_pos[:, -1] > 0
        N_sel = tgt_pos[:, -1] < 0
        assert p.shape[0] == src_pos.shape[0] and p.shape[1] == tgt_pos.shape[0]
        assert np.all(p[0, P_sel] == scale[0] * np.exp(-exponent[0] * dist[P_sel])) # Bipolar (pos) exponential dist.-dep. conn. prob.
        assert np.all(p[0, N_sel] == scale[1] * np.exp(-exponent[1] * dist[N_sel])) # Bipolar (neg) exponential dist.-dep. conn. prob.
        ...
        dist = np.sqrt(np.sum(tgt_pos**2, 1))
        assert p.shape[0] == src_pos.shape[0] and p.shape[1] == tgt_pos.shape[0]
        assert np.all(p[0, :] == 0.5 * (scale[0] * np.exp(-exponent[0] * dist) + scale[1] * np.exp(-exponent[1] * dist))) # Bipolar (pos/neg average) exponential dist.-dep. conn. prob.
...
def test_ConnProb4thOrderLinInterpnReducedModel():
    ...
    with setup_tempdir(__name__) as tempdir:
        ...
        p_table = pd.DataFrame(p_ref.flatten(), index=pd.MultiIndex.from_product([dr_pos, dz_pos], names=['dr', 'dz']), columns=['p'])
        model = test_module.AbstractModel.model_from_dict({'model': 'ConnProb4thOrderLinInterpnReducedModel'}, {'p_conn_table': p_table})

        # Check prob model access (at center positions)
        src_pos = np.zeros([1, 3])
        tgt_pos = np.array([p_table.index.get_level_values(0).to_numpy(), np.zeros(p_table.shape[0]), p_table.index.get_level_values(1).to_numpy()]).T
        p = model.apply(src_pos=src_pos, tgt_pos=tgt_pos)
...
def test_ConnProb5thOrderLinInterpnReducedModel():
    ...
    with setup_tempdir(__name__) as tempdir:
        ...
        p_table = pd.DataFrame(p_ref.flatten(), index=pd.MultiIndex.from_product([z_pos, dr_pos, dz_pos], names=['z', 'dr', 'dz']), columns=['p'])
        model = test_module.AbstractModel.model_from_dict({'model': 'ConnProb5thOrderLinInterpnReducedModel'}, {'p_conn_table': p_table})

        # Check prob model access (at center positions)
        for z_idx in range(len(z_pos)):
            z = z_pos[z_idx]
            src_pos = np.array([[0.0, 0.0, z]])
            tgt_pos = np.array([p_table[p_table.index.get_level_values(0) == z].index.get_level_values(1).to_numpy(), np.zeros(np.sum(p_table.index.get_level_values(0) == z)), z + p_table[p_table.index.get_level_values(0) == z].index.get_level_values(2).to_numpy()]).T
            p = model.apply(src_pos=src_pos, tgt_pos=tgt_pos)
