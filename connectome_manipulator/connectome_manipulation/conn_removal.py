...
def apply(
    ...
    if conn_mask_file is not None:
        ...
        log.log_assert(
            os.path.exists(conn_mask_file), f'Connection mask file "{conn_mask_file}" not found!'
        )
        ...
        src_conv = sps.csr_matrix(
            (np.arange(len(gids_src), dtype=int), (np.zeros(len(gids_src), dtype=int), gids_src))
        )
        dest_conv = sps.csr_matrix(
            (np.arange(len(gids_dest), dtype=int), (np.zeros(len(gids_dest), dtype=int), gids_dest))
        )
