...
def _get_tgt_morph(tgt_morph, morph_ext, node_id):
    """Access function (incl. transformation!), using specified format (swc/h5/...)"""
    return tgt_morph.get(node_id, transform=True, extension=morph_ext)
...
def apply(
    ...
    all_new_edges = connectome_wiring_wrapper(
        src_node_ids,
        src_pos,
        src_mtypes,
        src_class,
...
def init_edges_table(with_delay=True, from_table=None):
    ...
    if from_table is None:  # Create empty table
        ...
        if not np.all(
            [property_types[k] == v for k, v in all_new_edges[required_properties].dtypes.items()]
        ):
...
def connectome_wiring_wrapper(
    ...
    all_new_edges = init_edges_table(
    ...
    for tidx, tgt in enumerate(tgt_node_ids):
        ...
        sec_ind = np.hstack(
            [
                [-1],
                np.where(np.isin(morph.section_types, [nm.BASAL_DENDRITE, nm.APICAL_DENDRITE]))[0],
            ]
