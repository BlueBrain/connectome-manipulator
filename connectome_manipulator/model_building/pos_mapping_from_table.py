        ...
        for pidx, (pos, lbl, coo) in enumerate(zip(pos_list, lbl_list, coord_list)):
            if len(coo) == 2:
                if vidx == 0: # Only plot once, since only single 2D view
                    ax = fig.add_subplot(len(views_3d), len(pos_list), vidx * len(pos_list) + pidx + 1)
                    for lidx in range(num_layers):
                        pos_sel = pos[nrn_lay == lidx + 1, :]
                        plt.plot(pos_sel[:, 0], pos_sel[:, 1], '.', color=lay_colors[lidx, :], markersize=1.0, alpha=0.5, label=f'L{lidx + 1}')
                    ax.set_xlabel(coo[0])
                    ax.set_ylabel(coo[1])
            elif len(coo) == 3:
                ax = fig.add_subplot(len(views_3d), len(pos_list), vidx * len(pos_list) + pidx + 1, projection='3d')
                for lidx in range(num_layers):
                    pos_sel = pos[nrn_lay == lidx + 1, :]
