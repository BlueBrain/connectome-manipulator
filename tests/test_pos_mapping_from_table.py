...
def test_extract():
    ...
    with setup_tempdir(__name__) as tempdir:
        ...
        res = test_module.extract(c, pos_file, coord_names, coord_scale=None, nodes_pop_name=None, nodes_spec=None, zero_based_indexing=True, gid_column=None)
        ...
        res = test_module.extract(c, pos_file, coord_names, coord_scale=coord_scale, nodes_pop_name=None, nodes_spec=None, zero_based_indexing=True, gid_column=None)
        assert_array_equal(res['nrn_ids'], node_ids)
        ...
        pos_tab = pd.DataFrame(np.hstack([np.random.rand(N, len(coord_names)), np.arange(N).reshape([N, 1])]), columns=coord_names + ['gid'])
        pos_tab = pos_tab.astype({'gid': int})
        pos_file = os.path.join(tempdir, 'pos_tab_TEST.feather')
        pos_tab.to_feather(pos_file)

        res = test_module.extract(c, pos_file, coord_names, coord_scale=None, nodes_pop_name=None, nodes_spec=None, zero_based_indexing=True, gid_column='gid')
        assert_array_equal(res['nrn_ids'], node_ids)
        assert_array_equal(res['map_pos'], pos_tab[coord_names].loc[node_ids])
        assert_array_equal(res['nrn_pos'], nodes.positions(node_ids))
        assert_array_equal(res['nrn_lay'], nodes.get(node_ids, properties='layer'))
        ...
        pos_tab = pd.DataFrame(np.hstack([np.random.rand(N, len(coord_names)), np.arange(1, N + 1).reshape([N, 1])]), columns=coord_names + ['gid'])
        pos_tab = pos_tab.astype({'gid': int})
        pos_file = os.path.join(tempdir, 'pos_tab_TEST.feather')
        pos_tab.to_feather(pos_file)

        res = test_module.extract(c, pos_file, coord_names, coord_scale=None, nodes_pop_name=None, nodes_spec=None, zero_based_indexing=False, gid_column='gid')
        assert_array_equal(res['nrn_ids'], node_ids)
        assert_array_equal(res['map_pos'], pos_tab[coord_names].loc[node_ids])
        assert_array_equal(res['nrn_pos'], nodes.positions(node_ids))
        assert_array_equal(res['nrn_lay'], nodes.get(node_ids, properties='layer'))
