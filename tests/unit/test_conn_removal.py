...
def test_apply(manipulation):
    ...
    for pct in range(0, 101, 25):
        res = manipulation.apply(edges_table, nodes, None, amount_pct=pct)
    ...
    res = manipulation.apply(edges_table, nodes, None, min_syn_per_conn=2, amount_pct=100)
    ...
    res = manipulation.apply(edges_table, nodes, None, max_syn_per_conn=2, amount_pct=100)
    _, n_syn_conn = np.unique(res[["@source_node", "@target_node"]], axis=0, return_counts=True)
    assert np.all(n_syn_conn > 2)

    # Check that connection size filtering works when both are enabled
    res = manipulation.apply(
        edges_table, nodes, None, min_syn_per_conn=2, max_syn_per_conn=2, amount_pct=100
    )
    _, n_syn_conn = np.unique(res[["@source_node", "@target_node"]], axis=0, return_counts=True)
    assert np.all(n_syn_conn != 2)

    # Check with empty selection, i.e. nothing to be removed
    res = manipulation.apply(edges_table, nodes, None, min_syn_per_conn=np.inf, amount_pct=100)
    _, n_syn_conn = np.unique(res[["@source_node", "@target_node"]], axis=0, return_counts=True)
