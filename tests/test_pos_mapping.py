...
def test_extract():
    ...
    for scale in np.random.rand(10, 3):
        res = test_module.extract(circuit, flatmap_path, xy_file, z_file, xy_scale=scale[:2], z_scale=scale[2])
    ...
    res = test_module.extract(circuit, flatmap_path, xy_file, z_file, xy_scale=[1.0, 1.0], z_scale=1.0, NN_only=True)
    assert_array_equal(nrn_ids, res['nrn_ids'])
    assert_array_equal(nrn_pos, res['nrn_pos'])
    ...
    zval_lin = griddata(np.floor(zidx) + 0.5, np.floor(zidx), zidx)[:, 2] # Data points are at bin centers
    assert np.any(np.isfinite(zval_lin)), 'ERROR: Linear interpolation on test data not possible!'
    zval_lin2 = np.interp(zidx[:, 2], np.sort(np.floor(zidx[:, 2])) + 0.5, np.sort(np.floor(zidx[:, 2]))) # Interpolate linearly using numpy
    assert_allclose(zval_lin[np.isfinite(zval_lin)], zval_lin2[np.isfinite(zval_lin)]) # Check that linearly interpolated along z-axis indeed
    zval[np.isfinite(zval_lin)] = zval_lin[np.isfinite(zval_lin)] # Use lin. interpolation if possible, otherwise keep NN

    res = test_module.extract(circuit, flatmap_path, xy_file, z_file, xy_scale=[1.0, 1.0], z_scale=1.0, NN_only=False)
    assert_array_equal(nrn_ids, res['nrn_ids'])
    assert_array_equal(nrn_pos, res['nrn_pos'])
    assert np.all([np.all(res['flat_pos'][:, i] == 1.0) for i in range(2)])
