...
def apply(
    ...
    progress_pct = np.round(100 * np.arange(len(tgt_node_ids)) / (len(tgt_node_ids) - 1)).astype(
        int
    )
    for tidx, tgt in enumerate(tgt_node_ids):
        ...
        if num_src_to_reuse < num_new:  # Generate new synapses/connections, if needed
            ...
            if (
                ...
                new_edges = duplicate_sample_synapses(
                    src_gen,
                    tidx,
                    edges_table,
                    nodes,
                ...
                new_edges = duplicate_randomize_synapses(
                    src_gen,
                    edges_table,
                    nodes,
                    syn_sel_idx,
...
def duplicate_sample_synapses(
    ...
    if (
        tgt_mtype in duplicate_sample_synapses.per_mtype_dict
        ...
        syn_sel_idx_mtype = np.logical_and(
            duplicate_sample_synapses.syn_sel_idx_type,
        ...
        if np.sum(syn_sel_idx_mtype) == 0:  # Ignore m-type, consider matching layer
            syn_sel_idx_mtype = np.logical_and(
                duplicate_sample_synapses.syn_sel_idx_type,
                np.isin(edges_table["@target_node"], tgt_node_ids[tgt_layers == tgt_layers[tidx]]),
            )
        ...
        duplicate_sample_synapses.per_mtype_dict[tgt_mtype] = {
...
def duplicate_randomize_synapses(
    src_gen, edges_table, nodes, syn_sel_idx, syn_sel_idx_tgt, tgt_id, delay_model, props_model
):
    ...
    if duplicate_randomize_synapses.reset:
        duplicate_randomize_synapses.props_sel = list(
            filter(
                lambda x: not np.any(
                    [
                        excl in x
        ...
        log.log_assert(
            np.all(np.isin(duplicate_randomize_synapses.props_sel, props_model.get_prop_names())),
            f"Required properties missing in properties model (must include: {duplicate_randomize_synapses.props_sel})!",
        )
    ...
    new_edges[duplicate_randomize_synapses.props_sel] = pd.concat(new_syn_props, ignore_index=True)[
        duplicate_randomize_synapses.props_sel
    ].to_numpy()
...
def assign_delays_from_model(
    delay_model, nodes, edges_table, src_new, src_syn_idx, syn_sel_idx=None
):
