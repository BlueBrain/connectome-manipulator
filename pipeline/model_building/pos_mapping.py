...
""" Extract position mapping from atlas space to flat space (2 files required: 1. xy mapping in a.u., 2. z (=depth) mapping in um) """
def extract(circuit, flatmap_path, flatmap_files, xy_scale=None, **_):
    ...
    # Load flatmap
    assert len(flatmap_files) == 2, 'ERROR: Two flatmap files required: 1. xy mapping in a.u., 2. z (=depth) mapping in um'
    flatmap_atlas = Atlas.open(flatmap_path)
    flatmap = flatmap_atlas.load_data(flatmap_files[0])
    depths = flatmap_atlas.load_data(flatmap_files[1])
    ...
    if xy_scale is None: # Scaling from a.u. to um
        xy_scale = [flatmap.voxel_dimensions[0], flatmap.voxel_dimensions[1]] # Assume same pixel size in flat space as voxel size in atlas
    else:
        assert np.array(xy_scale).size == 2 and np.all(np.array(xy_scale) > 0), 'ERROR: XY scale error!'
...
def plot(out_dir, nrn_ids, nrn_lay, nrn_pos, model, model_inputs, model_params, **_):
    ...
    plt.ylim((0, dist_max))
    plt.grid(True)
    plt.xlabel('Distance in atlas space (data) [um]')
    plt.ylabel('Distance in flat space (model) [um]')
