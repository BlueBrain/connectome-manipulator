...
from connectome_manipulator.connectome_manipulation.helper_functions import (
    get_gsyn_sum_per_conn,
    rescale_gsyn_per_conn,
)
...
def apply(
    ...
    ext_edges_config_file=None,
):
    """Add certain amount of synapses to existing connections, optionally keeping sum of g_syns per connection constant."""
    # Input checks
    log.log_assert(
        isinstance(amount, dict) and "type" in amount.keys() and "value" in amount.keys(),
        'Amount must be specified as dict with "type"/"value" entries!',
    )
    log.log_assert(
        amount["type"] in ["pct", "pct_per_conn", "rnd_per_conn", "minval_per_conn"],
        f'Synapse amount type "{amount["type"]}" not supported (must be "pct", "pct_per_conn", "rnd_per_conn", or "minval_per_conn")!',
    )
    ...
    log.log_assert(
        method in ["duplicate", "derive", "load"],
        f'Synapse addition method "{method}" not supported (must be "duplicate", "derive", or "load")!',
    )
    ...
    if (
        amount["type"] == "pct"
    ):  # Overall increase by percentage of total number of existing synapses between src and dest
        log.log_assert(
            amount["value"] >= 0.0, f'Amount value for type "{amount["type"]}" out of range!'
        )
        num_add = np.round(amount["value"] * num_syn / 100).astype(int)
    elif (
        amount["type"] == "pct_per_conn"
    ):  # Increase each connection by percentage of existing synapses
        log.log_assert(
            amount["value"] >= 0.0, f'Amount value for type "{amount["type"]}" out of range!'
        )
        num_add = np.round(amount["value"] * num_syn_per_conn / 100).astype(int)
    elif (
        amount["type"] == "rnd_per_conn"
    ):  # Increase each connection by random number of synapses within given range
        log.log_assert(
            not np.isscalar(amount["value"]) and len(amount["value"]) == 2,
            f'Amount value for type "{amount["type"]}" needs to be a range with two elements!',
        )
        log.log_assert(
            amount["value"][0] >= 0 and amount["value"][1] >= amount["value"][0],
            f'Amount value range for type "{amount["type"]}" out of range!',
        )
        num_add = np.random.randint(
            low=np.round(amount["value"][0]).astype(int),
            high=np.round(amount["value"][1]).astype(int) + 1,
            size=conns.shape[0],
        )
    elif (
        amount["type"] == "minval_per_conn"
    ):  # Increase until minimum target value of synapses per connection is reached
        log.log_assert(
            amount["value"] >= 0, f'Amount value for type "{amount["type"]}" out of range!'
        )
    ...
    log.info(
        f'Adding {np.sum(num_add)} synapses to {edges_table.shape[0]} total synapses (sel_src={sel_src}, sel_dest={sel_dest}, amount[type]={amount["type"]}, amount[value]={amount["value"]}, method={method}, rescale_gsyn={rescale_gsyn})'
    )
    ...
    if method == "derive":
        ...
        log.log_assert(
            ...
            f'Synaptic properties model file required for "{method}" method!',
        )
        log.log_assert(
            os.path.exists(props_model_file), "Synaptic properties model file not found!"
        )
        ...
        props_model = model_building.get_model(
            props_model_dict["model"],
            props_model_dict["model_inputs"],
            props_model_dict["model_params"],
        )
    ...
    if method == "load":
        ...
        log.log_assert(
            ext_edges_config_file is not None,
            f'External connectome edges file required for "{method}" method!',
        )
        log.log_assert(
            os.path.exists(ext_edges_config_file), "External connectome edges file not found!"
        )
        ...
        log.log_assert(
            len(c_ext.edges.population_names) == 1,
            "Only a single edge population per file supported as external connectome!",
        )
        ...
        log.log_assert(
            edges_ext.source.name == nodes[0].name and edges_ext.target.name == nodes[1].name,
            f"External nodes populations inconsistent ({edges_ext.source.name}->{edges_ext.target.name} instead of {nodes[0].name}->{nodes[1].name})!",
        )
        nodes_ext = [edges_ext.source, edges_ext.target]
        log.log_assert(
            np.all([np.array_equal(n.ids(), n_ext.ids()) for (n, n_ext) in zip(nodes, nodes_ext)]),
            "External connectome not consistent with circuit nodes!",
        )

        # Extract external edges table between selected nodes
        edges_table_ext = edges_ext.afferent_edges(
            aux_dict["split_ids"], properties=list(edges_table.columns)
        )
        edges_table_ext = edges_table_ext[
            np.logical_and(
                np.isin(edges_table_ext["@source_node"], gids_src),
                np.isin(edges_table_ext["@target_node"], gids_dest),
            )
        ]
        num_syn_ext = edges_table_ext.shape[0]

        # Remove duplicate synapses that are already in circuit edges table
        edges_table_ext = edges_table_ext.append(edges_table.append(edges_table)).drop_duplicates(
            keep=False, ignore_index=True
        )
        log.info(
            f"Loaded external edges table with {edges_table_ext.shape[0]} synapses ({num_syn_ext} synapses initially; {num_syn_ext - edges_table_ext.shape[0]} duplicates dropped)"
        )
        ...
        conns_ext, syn_conn_idx_ext = np.unique(
            edges_table_ext[["@source_node", "@target_node"]], axis=0, return_inverse=True
        )
        ...
        log.log_assert(
            np.sum(syn_sel_idx_ext) > 0,
            "No external synapses corresponding to existing connections found!",
        )
    ...
    if np.sum(num_add) > 0:
        ...
        if method in ("duplicate", "derive"):
            ...
            if (
                method == "derive"
            ):  # Derive from existing synapses (duplicate and randomize certain properties based on a model)
                ...
                model_props = sorted(
                    props_model(None, None, None, None)
                )  # List of properties in the model
                model_props = sorted(
                    np.intersect1d(edges_table.keys(), model_props)
                )  # Select model props which are part of the edges table

                # Randomize within connections
                new_conns, new_syn_conn_idx = np.unique(
                    new_edges[["@source_node", "@target_node"]], axis=0, return_inverse=True
                )
                for new_idx in range(new_conns.shape[0]):
                    ...
                    prop_means = [
                        np.mean(edges_table[syn_sel_idx].loc[syn_conn_idx == conn_idx, p])
                    ...
                    prop_stds = [
                        props_model(
                            p,
                            src_mtypes[new_syn_conn_idx == new_idx].iloc[0],
                            tgt_mtypes[new_syn_conn_idx == new_idx].iloc[0],
                            "std-within",
                        )
                        for p in model_props
                    ]
                    ...
                    new_vals = (
                        np.random.randn(np.sum(new_syn_conn_idx == new_idx), len(model_props))
                        * prop_stds
                        + prop_means
                    )
            ...
            if np.isscalar(num_add):  # Overall number of external synapses to add
                ...
                if num_syn_sel < num_add:
                    ...
                    log.warning(
                        f"{num_add - num_syn_sel} of {num_add} synapses could not be added, since not enough external synapses provided!"
                    )
                else:
                    sel_ext = np.random.choice(
                        np.where(syn_sel_idx_ext)[0], num_add, replace=False
                    )  # Random sampling from external synapses without replacement
                ...
                for cidx, num in enumerate(num_add):
                    if num == 0:  # Nothing to add for this connection
                        continue
                    conn_sel_ext = (
                        syn_conn_idx_ext == conn_map[cidx]
                    )  # Select all external synapses belonging to a given connection
                    num_syn_sel = np.sum(conn_sel_ext)
                    if num_syn_sel < num:
                        sel_ext[ext_idx[cidx] : (ext_idx[cidx] + num_syn_sel)] = np.where(
                            conn_sel_ext
                        )[
                            0
                        ]  # Add all available synapses
                    else:
                        sel_ext[ext_idx[cidx] : ext_idx[cidx + 1]] = np.random.choice(
                            np.where(conn_sel_ext)[0], num, replace=False
                        )  # Random sampling from external synapses per connection without replacement
                if np.sum(sel_ext == -1) > 0:
                    log.warning(
                        f"{np.sum(sel_ext == -1)} of {np.sum(num_add)} synapses could not be added, since not enough external synapses provided!"
                    )
