    ...
    for i, sgid in enumerate(node_ids):
        for j, tgid in enumerate(node_ids):
            if (sgid, tgid) in edge_groups.groups:
                assert_approx_equal(gsyn_table[i, j],
                                    edge_groups.get_group((sgid,tgid)).conductance.sum())
...
def test_rescale_gsyn_per_conn():
    ...
    mask = np.logical_and(np.in1d(edges_table['@source_node'], node_ids),
                          np.in1d(edges_table['@target_node'], node_ids))

    assert_array_equal(edges_table.conductance[mask],
                       edges_table_orig.conductance[mask] / ratio)

    assert_array_equal(edges_table.conductance[~mask],
                       edges_table_orig.conductance[~mask])
