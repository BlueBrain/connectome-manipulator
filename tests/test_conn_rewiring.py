...
def test_apply():
    ...
    def check_all_to_all(ref, res, nodes, props_model=None):
        ...
        tgt_sel = np.unique(ref['@target_node']) # Consider only target node with existing synapses; for all others, no rewiring is possible!!
        ...
        for sidx, s in enumerate(src_sel):
            for tidx, t in enumerate(tgt_sel):
                cnt_mat[sidx, tidx] = np.sum(np.logical_and(res['@source_node'] == s, res['@target_node'] == t))
                if props_model is not None:
                    cnt_mat_model[sidx, tidx] = props_model.draw(prop_name='n_syn_per_conn', src_type=nodes[0].get(s, properties='mtype'), tgt_type=nodes[1].get(t, properties='mtype'))[0]
        ...
        if props_model is not None:
            assert np.all(cnt_mat[~eq_mat] == cnt_mat_model[~eq_mat]), 'ERROR: Number of synapses per connection not consistent with model!'
    ...
    def check_randomization(ref, res, props_model):
        ...
        for i in range(res.shape[0]):
            ...
            for p in np.setdiff1d(props_model.get_prop_names(), 'n_syn_per_conn'):
                assert r[p] == props_model.draw(prop_name=p, src_type=src_type, tgt_type=tgt_type)[0] # Assuming constant model!!
    ...
    for syn_class in ['EXC', 'INH']:
        ...
        with pytest.raises(AssertionError, match=re.escape('Keeping indegree not possible since connection probability zero!')):
            res = test_module.apply(edges_table.copy(), nodes, aux_dict, syn_class=syn_class, prob_model_file=prob_model_file, delay_model_file=delay_model_file, sel_src=None, sel_dest=None, amount_pct=pct, keep_indegree=True, reuse_conns=True, gen_method=None, props_model_file=None, pos_map_file=None)
        ...
        check_unchanged(edges_table, res, nodes, syn_class) # Check that non-selected connections unchanged
        ...
        for i_split, split_ids in enumerate(split_ids_list):
            ...
            res_list.append(test_module.apply(edges_table_split, nodes, aux_dict_split, syn_class=syn_class, prob_model_file=prob_model_file, delay_model_file=delay_model_file, sel_src=None, sel_dest=None, amount_pct=pct, keep_indegree=False, reuse_conns=False, gen_method='duplicate_sample', props_model_file=None, pos_map_file=None))
        ...
        res = test_module.apply(edges_table.copy(), nodes, aux_dict, syn_class=syn_class, prob_model_file=prob_model_file, delay_model_file=delay_model_file, sel_src=None, sel_dest=None, amount_pct=pct, keep_indegree=False, reuse_conns=False, gen_method='duplicate_randomize', props_model_file=props_model_file, pos_map_file=None)
        ...
        for i_split, split_ids in enumerate(split_ids_list):
            aux_dict_split = {'N_split': len(split_ids_list), 'i_split': i_split, 'split_ids': split_ids}
            edges_table_split = edges_table[np.isin(edges_table['@target_node'], split_ids)].copy()
            res_list.append(test_module.apply(edges_table_split, nodes, aux_dict_split, syn_class=syn_class, prob_model_file=prob_model_file, delay_model_file=delay_model_file, sel_src=None, sel_dest=None, amount_pct=pct, keep_indegree=False, reuse_conns=False, gen_method='duplicate_randomize', props_model_file=props_model_file, pos_map_file=None))
        res = pd.concat(res_list, ignore_index=True)
