...
def test_extract():
    ...
    tgt_pos = edges_table[['afferent_center_x', 'afferent_center_y', 'afferent_center_z']].to_numpy()
    dist = np.sqrt(np.sum((src_pos - tgt_pos)**2, 1))
    bins = np.arange(0, max_range_um + 1, bin_size_um)
    delays = [edges_table[np.logical_and(dist >= bins[idx], dist < bins[idx + 1])]['delay'].to_numpy() for idx in range(len(bins) - 1)]

    res = test_module.extract(c, bin_size_um, max_range_um, sel_src=None, sel_dest=None, sample_size=None)
