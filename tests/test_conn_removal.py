...
def test_apply():
    ...
    res = test_module.apply(edges_table, nodes, None, min_syn_per_conn=2, amount_pct=100)
    ...
    res = test_module.apply(edges_table, nodes, None, max_syn_per_conn=2, amount_pct=100)
    _, n_syn_conn = np.unique(res[["@source_node", "@target_node"]], axis=0, return_counts=True)
    assert np.all(n_syn_conn > 2)
    ...
    res = test_module.apply(edges_table, nodes, None, min_syn_per_conn=np.inf, amount_pct=100)
    _, n_syn_conn = np.unique(res[["@source_node", "@target_node"]], axis=0, return_counts=True)
